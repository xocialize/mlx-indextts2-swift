...
| `engine` | the consumer path — `MLXServeEngine()` (default `.permissiveOnly`) register → `needsDownload` → `prepare` (engine-executed materialization from mlx-community into `--store`) → three `run`s → evict; exactly the Studio's VoiceLane sequence | **PASS** — registered, 0 license advisories; materialized 4.2 GB + loaded in 110 s; en 4.34 s / −26.8 dBFS, happy `targetDuration` 4.0 → 3.99 s / −21.7 dBFS, zh 3.51 s / −26.2 dBFS |
